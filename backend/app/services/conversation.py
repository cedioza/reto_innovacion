...
class ConversationService:
    ...
    def list_all(self) -> list[ConversationResponse]:
        ...
        Se queda solo con el documento `session` de cada item devuelto por
        `ConversationRepository.list_all()` (que además trae `canal` y las
        marcas de tiempo, usadas por la vista de clientes).
        """
        return [item["session"] for item in self._repo.list_all()]
