...
- **Flujo paralelo (worktrees):** **`/launch-plan <plan>`** ejecuta un plan ya generado de punta a punta (sin gates, commits del orquestador) en un worktree aislado `../worktrees/plan-<id>` con rama `plan/<id>` — hasta 3 en simultáneo (una sesión por plan); **`/merge-plans <ramas>`** las integra a master en orden de lanzamiento, resolviendo conflictos con subagente + suite verde. Estado en `.claude/state/plans-launched.jsonl`. Ver [.claude/parallel/README.md](.claude/parallel/README.md).
